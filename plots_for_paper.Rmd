...
plot_performance = data.frame(
  method = c("brute_force", "simulated_annealing"),
  ...
  mutate(method = factor(method, levels = c("brute_force", "simulated_annealing"))) %>%
...
```{r}
plot_runtime = data.frame(method = c("brute_force", "simulated_annealing"),
           time = c(sum(runtime_brute_force), sum(runtime_optimal))) %>%
  mutate(method = factor(method, levels = c("random", "brute_force", "simulated_annealing"))) %>%
  ggplot(aes(x = method, y = time, fill = method)) +
  geom_col() +
  #scale_fill_manual(values = c("brute_force" = scales::hue_pal()(3)[2], "simulated_annealing" = scales::hue_pal()(3)[3])) +
  theme(axis.text.x = element_blank(),#element_text(angle = 45, hjust = 1),
        axis.title.x = element_blank())
...
```{r}
scaleFactor <- max(optimal_layout[['optimisation_data']]$objective_value) / max(optimal_layout[['optimisation_data']]$temperature)
...
ggsave("../results/plots/optimisation_for_schematic.pdf", width = 7, height = 5)
```
